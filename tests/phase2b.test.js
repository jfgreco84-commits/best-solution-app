...
R.section('=== FIXTURE SANITY ===');
{
  ...
  chk('the app carries the real verified preconditions', w.ctx.P2B_PRECOND,
      {deviceFp:'bb3ca777/86919',cloudFp:'67c10862/80102',device:{shows:35,days:75},cloud:{shows:32,days:71}});
}
...
R.section('=== 3. THE EXACT AUTHORIZATION MANIFEST ===');
{
  ...
  chk('manifest names three shows by stable id', M.shows.map(x=>x.id), [
    'sh_auto_wholebeadshowmilwauk_1787264415031803',
    'sh_auto_wonderfulworldofwedd_1787264415031379',
    'sh_auto_partyonthepavement_1787264415032841']);
...
// the exact stored spelling is ACCEPTED
{
  ...
  chk('and the authorized record carries it',
      r.op.auth.shows.find(x=>x.id==='sh_auto_wholebeadshowmilwauk_1787264415031803').record.name,
